 ...
 *  scrivere bootloader su micro brickato
 *  caricare ArduinoISP originale
 *  riscrivere bootloader su micro brickato
 *  caricare sketch desiderato
 *  
 *  se non funziona leggere tutta la procedura
 *  
 */
...
/*Here's how I un-bricked them ...
    ...
    Next, I selected a known good board selection for the ATtiny85 at 1 MHz and burned the boot loader set the fuses. You may encounter the following error:

         avrdude: stk500_paged_write(): (a) protocol error, expect=0x14, resp=0x00
         avrdude: stk500_cmd(): programmer is out of sync

    Just keep going, the rest is easy.
    Now i went to the original ArduinoISP sketch and uploaded that to the nano, so out with the cap again, upload, cap back in.
    ...
    I tested it out with the Blink sketch (led = 4) to verify everything was peachy. Blink blink blink.
